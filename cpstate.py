...
class IncompFluid(CP.AbstractState):
    ...
    def __new__(cls,name):
        instance = super().__new__(cls,"INCOMP",name)
        return instance
...
class Fluid(CP.AbstractState):
    ...
    def __init__(self,name):
        ...
        self.trv = State('pT',Pt,Tt,phase='V',desc='Trip. (V)')                
        return
    def satline(self,props='sT',npt=600,Tmin=None):
    ...
    def crit_isobar(self,props='sT',npt=600,Tmin=None):
        """Creates arrays of properties along the critical isobar. props specifies the properties to be returned. These are 
        returned as numpy arrays within an "ioutils" Dictionary. If Tmin is not
        specified then it is set to the triple point temperature."""
        if not Tmin: Tmin = self.trl.T
        Tmax = self.crt.T*4
        prop = np.zeros((len(props),npt))
        p_crit = self.p_critical()
        sat_max_s = State('TC', Tmin, 'g', fluid=self)
        sat_min_s = State('TC', Tmin, 'f', fluid=self)
        max_h = State('ps', p_crit, sat_max_s.s)
        min_h = State('ps', p_crit, sat_min_s.s)
        s = np.linspace(sat_min_s.s, sat_max_s.s, npt)
        h = np.linspace(min_h.h, max_h.h, npt)
        T = np.linspace(Tmin, Tmax, npt)
        state = State('ref',fluid=self,desc='dummy')
        count = 0
        retry = False
...
class State():
    ...
    def __str__ (self):
        str = "%s %s state " %(self.fluid.name,self.desc)
